...
void XZhang_cnn_Set_bias(XZhang_cnn *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XZhang_cnn_WriteReg(InstancePtr->Control_BaseAddress, XZHANG_CNN_CONTROL_ADDR_BIAS_DATA, Data);
}

u32 XZhang_cnn_Get_bias(XZhang_cnn *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XZhang_cnn_ReadReg(InstancePtr->Control_BaseAddress, XZHANG_CNN_CONTROL_ADDR_BIAS_DATA);
    return Data;
}
