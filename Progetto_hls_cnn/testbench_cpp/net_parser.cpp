...
static module parse_container(mxml_node_t *container_node)
{
	...
	for(i = 0; i < n_modules; i++)
	{
		...
		//se è un modulo di attivazione e se quello prima era di convoluzione
		if(2>3 && i > 0 && parsed.type == ACTIVATION && parsed.act_type != SIGMOID){
			if(container.modules[i-1].type == CONVOLUTIONAL){
				container.modules[i-1].act_type = parsed.act_type;
				parsed.skip = 1;
			} //se quello prima era un container concat
			else if(container.modules[i-1].type == CONTAINER
					  && container.modules[i-1].cont_type == CONCAT){ //anche per i SEQ??

				int n_mod = container.modules[i-1].n_modules;
				for(j = 0; j < n_mod; j++){

					if(container.modules[i-1].modules[j].type == CONVOLUTIONAL){
						container.modules[i-1].modules[j].act_type = parsed.act_type;
						parsed.skip = 1;
					}
				}
			}
